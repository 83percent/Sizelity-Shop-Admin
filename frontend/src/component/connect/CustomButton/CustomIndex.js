...
const CustomButtonDesign = () => {
    ...
    return (
        <article id="custom-design">
            ...
            <h3>HTML 코드</h3>
            {/* <textarea readOnly value={`<button onclick="location.href='https://www.sizelity.com/compare?domain='+encodeURIComponent(location.href)" style="all:unset; position: fixed; z-index: 999; bottom: ${position.vertical || 12}px; ${position.align === "right" ? "right" : "left"}: ${position.horizontal || 12}px"><div style="cursor:pointer;width: ${options.icon.size}rem; height: ${options.icon.size}rem; display: flex; align-items: center; justify-content: center; border-radius: 50%; background-color: ${options.icon.color || "#00966B"};"><svg xmlns="http://www.w3.org/2000/svg" height="${iconSize}rem" viewBox="0 0 24 24" width="${iconSize}rem" fill="${baseColor || "#ffffff"}">${options.icon.type === "ruler" ? '<path d="M0 0h24v24H0z" fill="none"/><path d="M21 6H3c-1.1 0-2 .9-2 2v8c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm0 10H3V8h2v4h2V8h2v4h2V8h2v4h2V8h2v4h2V8h2v8z"/>' : '<path d="M0 0h24v24H0z" fill="none"/><path d="M21.41 11.58l-9-9C12.05 2.22 11.55 2 11 2H4c-1.1 0-2 .9-2 2v7c0 .55.22 1.05.59 1.42l9 9c.36.36.86.58 1.41.58.55 0 1.05-.22 1.41-.59l7-7c.37-.36.59-.86.59-1.41 0-.55-.23-1.06-.59-1.42zM5.5 7C4.67 7 4 6.33 4 5.5S4.67 4 5.5 4 7 4.67 7 5.5 6.33 7 5.5 7z"/>'}</svg></div></button>`}>
